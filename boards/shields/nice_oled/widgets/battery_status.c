...
#include "battery_status.h"
    ...
    #define PERIPHERAL_COUNT CONFIG_ZMK_SPLIT_BLE_PERIPHERAL_COUNT
#else
    #define PERIPHERAL_COUNT 0
#endif
...
static lv_color_t battery_image_buffer[PERIPHERAL_COUNT + 1][5 * 8];
...
static void set_battery_symbol(lv_obj_t *widget, struct battery_state state) {
    if (state.source >= PERIPHERAL_COUNT + 1) {
        return;
    }
    ...
    if (state.source == 0) {
        ...
        lv_label_set_text_fmt(label, "P:%3u%%", state.level);
    }
...
int zmk_widget_battery_status_init(struct zmk_widget_battery_status *widget, lv_obj_t *parent) {
    ...
    for (int i = 0; i < PERIPHERAL_COUNT + 1; i++) {
